...
export default function Home() {
  ...
  // == BAGIAN STRAPI: State untuk menyimpan data project ==
  ...
  return (
    <main className="bg-white min-h-screen font-sans">
        ...
        <div className="relative z-10 w-full px-6 md:px-12 flex flex-col h-full">
          ...
          >
            ...
            <div className="flex justify-center gap-2 md:gap-x-6 mt-12 mb-16 bg-white md:bg-transparent rounded-full  shadow-lg md:shadow-transparent z-9">
              ...
              <a
                href="URL_LINKEDIN_KAMU"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="LinkedIn"
                className="z-10 group flex items-center justify-center size-14 md:bg-white md:rounded-full md:shadow-lg transition duration-150 ease-in-out hover:shadow-xl hover:scale-105"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 448 512"
                  className="size-8 fill-zinc-700 transition duration-150 group-hover:fill-zinc-900"
                >
              ...
              <a
                href="https://github.com/bobbyrahmanh"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="GitHub"
                className="z-10 group flex items-center justify-center size-14 md:bg-white md:rounded-full md:shadow-lg transition duration-150 ease-in-out hover:shadow-xl hover:scale-105"
              ...
              <a
                href="https://www.instagram.com/bobbyrh/"
                target="_blank"
                rel="noopener noreferrer"
              ...
              <a
                href="mailto:bobbyrahmanh123@gmail.com"
                aria-label="Email"
                className="group flex items-center justify-center size-14 md:bg-white md:rounded-full md:shadow-lg transition duration-150 ease-in-out hover:shadow-xl hover:scale-105"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 512 512"
                  className="z-10 size-8 fill-zinc-700 transition duration-150 group-hover:fill-zinc-900"
                >
      ...
      {/* == PROJECTS SECTION (PENGGANTI BLABLA) == */}
      <section
        id="projects"
        className="max-w-full mx-auto px-6 py-20 bg-linear-to-b from-white via-[#ffefff] to-[#dfebff] rounded-t-[40px] mt-20"
      >
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-20">
              {projects && projects.length > 0 ? (
                projects.map((project) => {
                  ...
                  const imagePath =
                    data.DesktopImage?.data?.attributes?.url ||
                    data.DesktopImage?.attributes?.url ||
                    data.DesktopImage?.url;

                  const desktopUrl = imagePath
                    ? `${STRAPI_URL}${imagePath}`
                    : null;

                  console.log(`Project: ${data.Title}`, {
                    rawMediaData: data.DesktopImage,
                    finalUrl: desktopUrl,
                  });
                  ...
                  return (
                    <div
                      key={id}
                      className="bg-white border border-[#ffffff] rounded-3xl p-8 text-black flex flex-col shadow-xl hover:shadow-2xl transition-all duration-300"
                    >
                      ...
                      <div className="relative w-full aspect-video bg-[#c8b2e6] rounded-2xl overflow-hidden mb-8 border border-white/5 shadow-inner group">
                        {desktopUrl ? (
                          <img
                            ...
                            alt={data.Title}
                            onError={(e) => {
                              console.error('Gambar gagal load:', desktopUrl);
                              e.currentTarget.style.display = 'none';
                            }}
                          />
                        ) : (
                          <div className="flex flex-col items-center justify-center h-full text-gray-500 bg-[#c8b2e6]">
                            <span className="text-4xl mb-2">🖼️</span>
                            <p className="text-xs italic">
                              No DesktopImage Found in API
                            </p>
                            <p className="text-[10px] mt-2 text-gray-600">
                              Check ?populate=* in lib/strapi.js
                            </p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="text-center mb-8">
                        <a
                          href={data.LiveUrl || '#'}
                          target="_blank"
                          className="text-[#3042cc] font-semibold hover:text-[#001eff] transition-colors flex items-center justify-center gap-2"
                        >
                          {data.LiveUrl?.includes('github.com')
                            ? '📂 View Source'
                            : '🌐 Live Website'}
                        </a>
                      </div>
                      ...
                      <div className="pt-6 border-t border-gray flex justify-between items-center text-[10px] text-gray font-mono">
                        <span className="bg-[#e4e4e4] px-2 py-1 rounded text-gray">
                          {data.TechStack || 'Stack N/A'}
                        </span>
                ...
                <p className="text-[#9e9e9e] text-center col-span-2">
                  Loading data...
                </p>
              )}
            </div>
      ...
      <footer
        id="footer"
        className="w-full bg-[#ffffff] border-t border-[#d4d4d4]"
      >
        <div className="flex flex-row max-w-[90%] mx-auto py-10 justify-between items-center">
          <p className="text-black text-[15px] font-semibold">
            © 2026 Bobby Rahman Hartanto. All rights reserved.
          </p>
          <div className="flex gap-4">
            <span className="text-xs text-gray-400">
              Built with Next.js, Strapi & Tailwind
            </span>
          </div>
        </div>
      </footer>
